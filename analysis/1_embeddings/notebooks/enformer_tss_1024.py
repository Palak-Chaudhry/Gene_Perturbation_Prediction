...
with torch.no_grad():
    for batch_idx, data in tqdm(enumerate(test_loader), total=len(test_loader), desc="Extracting features"):
        ...
        # Get embeddings from trunk (before the final head) - 3072 dimensions
        # This extracts richer representations than the 32-dim head output
        trunk_output = feature_extractor.embedding(curr_data)  # Shape: (batch, 3072, seq_len)
        batch_embeddings = trunk_output.mean(dim=2).detach().cpu().numpy()  # Shape: (batch, 3072)
...
if SAVE:
    output_file = "../../../embeddings/embeddings_enformer_tss.npy"
...
# Define paths
fasta_file = "../../../genomic_sequences/TSS.fa"
embeddings_file = "../../../embeddings/embeddings_enformer_tss_full.npy"
metadata_dir = "../../../embeddings"
output_file = os.path.join(metadata_dir, "enformer_gene_names.txt")
